...
int maxSuf_plus(int* A, int* P, int pk, int ai, int z) {
    int suf = 0;
    for (int i = ai; i > ai - k - z; i--) {
        if (A[i] == P[pk]) {
            suf++;
            pk--;
            if (pk < 0) break;
...
int DP_CLCSt_plus(int* A, int* B, int* P, int m, int n, int r, int k) {

    // 0 Initialize
    int*** Benson1 = new int** [r + 1];  // 三维 CLCSt matrix
    for (int i = 0; i <= r; i++) {
    ...
    // k = 0
    for (int i = k - 1; i < m; i++) {
        if (kmatch1[i][k - 1] == k) Benson1[0][i + 1][k] = k;
        else Benson1[0][i + 1][k] = Benson1[0][i][k];
    }
    for (int i = k - 1; i < n; i++) {
        if (kmatch1[k - 1][i] == k) Benson1[0][k][i + 1] = k;
    ...
    for (int i = k + 1; i <= m; i++) {
        for (int j = k + 1; j <= n; j++) {
            ...
            for (int z = 0; z < k; z++) {
                // cout << "kmatch1[i][j] = " << kmatch1[i - 1][j - 1] << " and k = " << k << ", z = " << z << endl;
                // if (i, j) is a k-plus-match
                if (kmatch1[i - 1][j - 1] >= k + z) Benson1[0][i][j] = max(Benson1[0][i - k - z][j - k - z] + k + z, Benson1[0][i][j]); 
            }
        }
    }
    // k > 0
        ...
        for (int i = 1; i <= m; i++) {
            for (int j = 1; j <= n; j++) {
                ...
                if (kmatch1[i - 1][j - 1] >= k) {
                    for (int z = 0; z < k; z++) {
                        if (kmatch1[i - 1][j - 1] >= k + z) {
                            int maxSuff = maxSuf_plus(A, P, h - 1, i - 1, z);
                            Benson1[h][i][j] = max(Benson1[h - maxSuff][i - k - z][j - k - z] + k + z, Benson1[h][i][j]); // if (i, j) is a k-plus-match
                        }
                    }
                }
            }
